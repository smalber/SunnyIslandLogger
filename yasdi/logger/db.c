...
void SIdbInit(char * iniFilename, int SIchannelCount, char SIdbColumns[][MAX_COLUMN_NAME_LEN], char SIdbTypes[][MAX_COLUMN_TYPE_LEN])
{
    ...
    strcpy(_iniFilename, iniFilename);
    _SIchannelCount = SIchannelCount;
	...
	if (mysql_real_connect(_conn, server, username, password, database, 0, NULL, 0) == NULL) {
		...
		if (errno != 1049 && errno != 1045) {
			...
			printf("\nCongratulations. A new database called '%s' has been created, with the table 'SI_values'.\n\
A new user '%s' has also been created and was granted access to the new DB.\n\
It is now strongly recommended to edit yasdi.ini to remove the\n\
root_username/root_password information.\n\nWaiting 20 seconds...\n", database, username);
			fflush(stdout);
	...
	// Prepare query
	sprintf(query, "CREATE TABLE IF NOT EXISTS SI_values (logdate DATE, logtime TIME");
	for (SIchannelNr = 0; SIchannelNr < SIchannelCount; SIchannelNr++) {
		strcat(query, ", ");
		strcat(query, SIdbColumns[SIchannelNr]);
		strcat(query, " ");
		strcat(query, SIdbTypes[SIchannelNr]);
	}
	strcat(query, ")");
...
int insertValues(int channelCount, char dbColumns[][MAX_COLUMN_NAME_LEN], char channelValues[][MAX_CHANNEL_VALUE_LEN]) {
    ...
    while (errno != 0) {
        ...
        if (errno != 0) {
            printLog(LEVEL_WARNING, "Error %u performing insert into SB Database: %s.\n", mysql_errno(_conn), mysql_error(_conn));
            reconnect();
            sleep(1);
        }
    }
...
int SIinsertValues(int SIchannelCount, char SIdbColumns[][MAX_COLUMN_NAME_LEN], char SIchannelValues[][MAX_CHANNEL_VALUE_LEN]) {
	...
	// Prepare query
	sprintf(query, "INSERT INTO SI_values (logdate, logtime");
	for (SIchannelNr = 0; SIchannelNr < SIchannelCount; SIchannelNr++) {
		strcat(query, ", ");
		strcat(query, dbColumns[SIchannelNr]);
	}
	strcat(query, ") VALUES ('");
	strcat(query, logdate);
	strcat(query, "', '");
	...
	for (SIchannelNr = 0; SIchannelNr < SIchannelCount; SIchannelNr++) {
		strcat(query, ", '");
		strcat(query, SIchannelValues[SIchannelNr]);
		strcat(query, "'");
	}
	strcat(query, ")");
    ...
    while (errno != 0) {
        ...
        if (errno != 0) {
            printLog(LEVEL_WARNING, "Error %u performing insert into SI Database: %s.\n", mysql_errno(_conn), mysql_error(_conn));
            reconnect();
            sleep(1);
        }
    }
